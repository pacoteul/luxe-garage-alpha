...
export default function Dashboard() {
  ...
  const runDesigner = async (leadId: number) => {
    ...
    try {
      ...
      alert("Erreur réseau lors de la création de la maquette.");
    }
                        ...
                        <td className="p-5 text-slate-400 font-mono text-sm space-y-1">
                          {lead.phone_number ? (
                            <div className="flex items-center gap-1">
                              {lead.has_whatsapp ? (
                                <><span title="Mobile (WhatsApp)"><MessageCircle className="w-3 h-3 text-green-400"/></span> <span className="text-green-400/80">{lead.phone_number}</span></>
                              ) : (
                                <><span title="Fixe"><Phone className="w-3 h-3 text-slate-500"/></span> {lead.phone_number}</>
                              )}
                            </div>
                          ) : (
